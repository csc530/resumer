def "random job" [] {
    let start = fakedata date --limit 1
    {
        ...
        Company: (fakedata adjectives industry --limit 1)
        Description: (if (random bool) { (fakedata sentence | lines) } else { [] })
        Experience: (if (random bool) { (fakedata sentence | lines) } else { [] })
        StartDate: $start
...
def "random profile" [] {
    {
        ...
        Interests: (if (random bool) { (fakedata noun --limit (fakedata int --limit 1)) } else { [] })
        Website: (if (random bool) { (fakedata username domain.tld --limit 1 --separator .) })
        Objective: (if (random bool) { fakedata sentence | lines | str join })
        Location: (if (random bool) {fakedata city --limit 1})
        Languages: (if (random bool) { (fakedata noun --limit (fakedata int --limit 1)) } else { [] })
    }
}
...
def "populate db" [] {
    generate null {|_| {out: (random job), next: true}} | first 10 | upsert Description {|e| '[' + ($e.Descrition? | each {|i| if ($i | is-not-empty) {$'"($i)"'}} | str join ,) + ']'} | upsert Experience {|e| '[' + ($e.Experience? | each {|i| if ($i | is-not-empty) {$'"($i)"'}} | str join ,) + ']'} | into sqlite resume.db --table-name Jobs

...
def "populate db" [num?:int = 10] {
    generate null {|_| {out: (random job), next: true}} | first $num | upsert Description {|e| '[' + ($e.Description? | par-each {|i| if ($i | is-not-empty) {$'"($i)"'}} | str join ,) + ']'} | upsert Experience {|e| '[' + ($e.Experience? | par-each {|i| if ($i | is-not-empty) {$'"($i)"'}} | str join ,) + ']'} | into sqlite resume.db --table-name Jobs
    generate null {|_| {out: (random profile), next: true}} | first $num | upsert Languages {|e| '[' + ($e.Languages | par-each {|i| if ($i | is-not-empty) {$'"($i)"'}} | str join ,) + ']'} | update Interests {|e| '[' + ($e.Interests | par-each {|i| if ($i | is-not-empty) {$'"($i)"'}} | str join ,) + ']'} | into sqlite resume.db --table-name Profiles
    generate null {|_| {out: (random skill), next: true}} | first $num | into sqlite resume.db --table-name Skills
}
...
def "populate db" [] {
    ...
    generate null {|_| {out: (random profile), next: true}} | first 10
    #| upsert Languages {|e| '[' + ($e.Languages | each {|i| if ($i | is-not-empty) {$'"($i)"'}} | str join ,) + ']'} | update Interests {|e| '[' + ($e.Interests | each {|i| if ($i | is-not-empty) {$'"($i)"'}} | str join ,) + ']'}
    | into sqlite resume.db --table-name Profiles
